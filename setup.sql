...
BEGIN
	IF (EXISTS (SELECT * FROM Members M WHERE mid_in = M.mid AND M.m_type='customer'))
		IF(NOT ValidatePaymentMethodExists(mid_in))
            RETURN;
		IF(EXISTS (SELECT * FROM Reserved R WHERE pid_in = R.pid AND mid_in = R.mid AND R.quantity>0))
			UPDATE Reserved
            SET quantity = quantity-1
            WHERE pid_in = pid;
...
BEGIN
	IF ((SELECT COUNT(*) FROM Members M WHERE mid = M.mid AND M.m_type='needy')>1)
		IF(NOT StillNeedy(mid_in))
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Maximum plates (2) reached!';
            RETURN;
		ELSEIF(EXISTS (SELECT * FROM Purchased P WHERE pid_in = P.pid AND P.quantity>0))
			UPDATE Purchased
            SET quantity = quantity-1
            WHERE pid_in = pid;
...
BEGIN
	IF (EXISTS (SELECT * FROM Members M WHERE mid_in = M.mid AND M.m_type='donor'))
		IF(EXISTS (SELECT * FROM On_Sale OS WHERE pid_in = OS.pid AND OS.quantity>0))
			UPDATE On_Sale
            SET quantity = quantity-1
            WHERE pid_in = pid;
...
CREATE PROCEDURE ValidatePaymentMethodExists(mid INT)
BEGIN
...
CREATE PROCEDURE StillNeedy(mid INT)
BEGIN
